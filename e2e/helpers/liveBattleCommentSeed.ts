...
/**
 * Read the most recent `report_resolved` audit-log entry for a given
 * report id. Used by moderation E2E to verify that clicking Resolve
 * writes the correct resolver + reason into admin_audit_log.
 */
export async function readLatestReportResolvedAudit(reportId: string) {
  const admin = adminClient();
  const { data } = await admin
    .from("admin_audit_log")
    .select("id, actor_id, action, target_type, target_id, details, created_at")
