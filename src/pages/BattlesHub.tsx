...
import { Swords, Radio, ChevronRight, Sparkles } from "lucide-react";
import { isFeatureEnabled } from "@/lib/featureFlags";

export default function BattlesHub() {
  useSeoMeta({
    title: "Battles — CrownMe",
    description: "Choose your battle mode: post battles or live head-to-head.",
  });
  ...
  return (
    <AppShell>
      <div className="mx-auto max-w-lg px-4 pt-6 pb-24">
        ...
        <div className="mt-8 rounded-2xl border border-border/60 bg-card/40 p-4 flex items-start gap-3">
          <Sparkles size={16} className="text-primary mt-0.5 shrink-0" />
          <p className="text-xs text-muted-foreground leading-relaxed">
            <span className="font-semibold text-foreground">New here?</span> Post
            battles are the fastest way to earn a crown. Live battles let you go
            face-to-face with another creator.
          </p>
        </div>
      </div>
    </AppShell>
...
function ModeCard({
  ...
  const inner = (
    <div
      className={`relative overflow-hidden rounded-2xl border border-border/60 bg-card p-5 transition ${
