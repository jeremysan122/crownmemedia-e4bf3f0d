// Recipient picker for "Share to DM". Mirrors GiftDmPicker but trimmed to the
// share use-case: shows existing DM threads first, then followed users,
// supports username search. Filters out banned/suspended and blocked users.
...
type ThreadRow = { partnerId: string; partner: GiftRecipientCandidate; lastAt: string };

export default function DmSharePicker({
  open,
  onOpenChange,
  onPick,
  title = "Share via DM",
  ...
  onPick: (r: DmShareRecipient) => void;
  title?: string;
  subtitle?: string;
}) {
  const { user } = useAuth();
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      ((dms as unknown[]) || []).forEach((raw) => {
        const row = raw as {
          sender_id: string;
          receiver_id: string;
          created_at: string;
          sender?: { id: string; username: string; first_name?: string; last_name?: string; profile_photo_url?: string | null; verified?: boolean; is_banned?: boolean; is_suspended?: boolean };
          receiver?: typeof row.sender;
        };
        const isMineSender = row.sender_id === user.id;
        const partnerId = isMineSender ? row.receiver_id : row.sender_id;
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md bg-gradient-card border-border/70 p-0 overflow-hidden rounded-2xl">
        <VisuallyHidden>
          ...
          <DialogDescription>Pick a contact from your inbox or followers to share with.</DialogDescription>
        </VisuallyHidden>
          ...
          {(loading || searching) && list.length === 0 ? (
            ...
            <div className="py-10 text-center space-y-2">
              ...
              <p className="text-xs text-muted-foreground px-6">
                {hasQuery ? "No users matched that search." : "Start a conversation or follow creators to share via DM."}
              </p>
            </div>
          ) : (
            <>
