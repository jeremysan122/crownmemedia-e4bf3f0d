...
DECLARE
  ...
  v_main text; v_sub text; blocked boolean := false;
BEGIN
  ...
  END;
  RETURN QUERY SELECT 'post_crown_score_update_blocked_by_trigger'::text,
  ...
  RETURN QUERY SELECT 'authenticated_column_privilege_status'::text,
    'info'::text, format('has_column_privilege=%s', has_priv);
