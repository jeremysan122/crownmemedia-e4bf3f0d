...
-- Wave 6 — Post-battle: highlight cards + battler analytics.

-- Track peak concurrent viewers so post-battle analytics has a real number.
ALTER TABLE public.live_battles
  ADD COLUMN IF NOT EXISTS peak_viewers integer NOT NULL DEFAULT 0;

-- Lightweight RPC — client bumps peak from presence hook.
-- SECURITY DEFINER so any authenticated viewer can raise the ceiling without
-- needing UPDATE rights on live_battles. Only ever raises, never lowers.
CREATE OR REPLACE FUNCTION public.bump_live_battle_peak_viewers(
  _battle_id uuid,
  _count integer
) RETURNS void
LANGUAGE plpgsql
...
BEGIN
  ...
  IF _count IS NULL OR _count < 0 OR _count > 1000000 THEN RETURN; END IF;
  UPDATE public.live_battles
    SET peak_viewers = _count
    WHERE id = _battle_id
      AND _count > peak_viewers
      AND status IN ('live', 'ended');
END;
$$;

REVOKE ALL ON FUNCTION public.bump_live_battle_peak_viewers(uuid, integer) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.bump_live_battle_peak_viewers(uuid, integer) TO authenticated;

-- Highlight data for the results card: names, avatars, top 3 gifters,
-- and per-side gift totals. Callable by any authenticated user.
...
BEGIN
  ...
  SELECT id, host_id, opponent_id, host_votes, opponent_votes, winner_id,
         status, category_slug, region, peak_viewers, ended_at
  ...
  SELECT COALESCE(jsonb_agg(row_to_json(t)), '[]'::jsonb) INTO _top_gifters
    FROM (
      SELECT g.sender_id, p.username, p.display_name, p.avatar_url,
       ...
       GROUP BY g.sender_id, p.username, p.display_name, p.avatar_url
  ...
  RETURN jsonb_build_object(
    ...
    'peak_viewers', _b.peak_viewers, 'ended_at', _b.ended_at,
...
REVOKE ALL ON FUNCTION public.get_live_battle_highlight(uuid) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.get_live_battle_highlight(uuid) TO authenticated;

-- Battler analytics — self-only (or admin/moderator). Returns recent ended
-- battles with peak viewers, vote totals, gift revenue, and top supporter.
CREATE OR REPLACE FUNCTION public.get_battler_battle_analytics(
  _user_id uuid,
...
BEGIN
  ...
  SELECT COALESCE(jsonb_agg(row_to_json(r) ORDER BY r.ended_at DESC), '[]'::jsonb)
    ...
    ORDER BY b.ended_at DESC NULLS LAST
    LIMIT _limit
  ) r;
...
REVOKE ALL ON FUNCTION public.get_battler_battle_analytics(uuid, integer) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.get_battler_battle_analytics(uuid, integer) TO authenticated;
