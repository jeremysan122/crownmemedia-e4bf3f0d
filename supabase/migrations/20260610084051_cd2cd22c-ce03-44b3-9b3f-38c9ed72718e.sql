-- 1) Dedupe existing rows: keep newest snapshot_at per logical key
WITH ranked AS (
  SELECT id,
         row_number() OVER (
           PARTITION BY period, scope_type, scope_value, main_slug,
                        COALESCE(subcategory_slug, ''), user_id
...
-- 2) Rebuild unique constraint with NULLS NOT DISTINCT (PG15+)
ALTER TABLE public.category_rankings
  DROP CONSTRAINT IF EXISTS category_rankings_period_scope_type_scope_value_main_slug_s_key;

ALTER TABLE public.category_rankings
  ADD CONSTRAINT category_rankings_unique_key
  UNIQUE NULLS NOT DISTINCT
    (period, scope_type, scope_value, main_slug, subcategory_slug, user_id);

-- 3) Rewrite function: use LATERAL with LIMIT 1 so a stray duplicate
--    never blows up the entire upsert with cardinality_violation.
...
BEGIN
  ...
  SELECT n.period, n.scope_type, n.scope_value, n.main_slug, n.subcategory_slug,
    ...
    LEFT JOIN LATERAL (
      SELECT r2.rank
        FROM public.category_rankings r2
       WHERE r2.period = n.period
         AND r2.scope_type = n.scope_type
         AND r2.scope_value = n.scope_value
         AND r2.main_slug = n.main_slug
         AND r2.subcategory_slug IS NOT DISTINCT FROM n.subcategory_slug
         AND r2.user_id = n.user_id
       ORDER BY r2.snapshot_at DESC NULLS LAST
       LIMIT 1
    ) prev ON true
