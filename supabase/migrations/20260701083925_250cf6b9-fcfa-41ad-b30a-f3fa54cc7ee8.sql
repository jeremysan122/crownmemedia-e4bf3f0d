  ...
  base AS (
    SELECT pe.period, po.user_id, po.main_category_slug AS main_slug, po.subcategory_slug,
     ...
     WHERE po.main_category_slug IS NOT NULL
       ...
       AND po.parent_post_id IS NULL  -- exclude reposts: credit stays with original author
  ),
  scoped AS (
...
-- Re-snapshot immediately to clear repost-contaminated rows.
SELECT public.snapshot_category_ranks();
