...
DECLARE
  test_user uuid;
  fake_normal_uid uuid := gen_random_uuid();
  before_score int;
  after_score int;
  tmp_post_id uuid;
  has_priv boolean;
BEGIN
  ...
  IF test_user IS NULL THEN
    RETURN QUERY SELECT 'setup'::text, 'fail'::text, 'no normal user found'::text; RETURN;
  END IF;

  SELECT crown_score INTO before_score FROM public.profiles WHERE id = test_user;

  -- 1. Simulate a normal (non-admin, non-moderator) user attempting to update
  --    someone else's profile crown_score. We use a random UUID so the
  --    unrelated profile rate-limit trigger short-circuits (NEW.id <> auth.uid()).
  ...
  RETURN QUERY SELECT
    ...
    format('before=%s after=%s uid=%s', before_score, after_score, fake_normal_uid);
  ...
  -- 3. Posts trigger end-to-end (trigger itself sets the sync flag)
  ...
  -- 4. authenticated has no UPDATE privilege on posts.crown_score
