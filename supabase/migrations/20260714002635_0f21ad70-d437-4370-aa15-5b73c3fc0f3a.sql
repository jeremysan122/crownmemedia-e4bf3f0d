...
-- Helper: is_founder(uuid) — active, non-revoked grant
...
BEGIN
  ...
  WHERE ad.is_active = true
    -- Fix #3: hide founder-only rows from non-founders
    AND (ad.is_founder_only = false OR v_is_founder = true)
    -- Secrets: still hidden until user has any progress row
    AND (ad.is_secret = false OR p.status IS NOT NULL)
    AND (ad.starts_at IS NULL OR ad.starts_at <= now())
    -- Fix #4: allow expired seasonal rows if user completed them
    AND (
      ad.ends_at IS NULL
      OR ad.ends_at >= now()
