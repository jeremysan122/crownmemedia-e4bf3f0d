...
AS $$
BEGIN
  -- Backward compatibility for older clients/rows that used the legacy
  -- subcategory slug "overall" under Royal Crowns. The current canonical
  -- topic slug is "overall-crown".
  IF NEW.main_category_slug = 'royal-crowns'
     AND NEW.subcategory_slug = 'overall' THEN
    NEW.subcategory_slug := 'overall-crown';
  END IF;
