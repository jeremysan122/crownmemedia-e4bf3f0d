
-- Wave 8.2a runtime proof harness. All three functions are dropped immediately after use.

CREATE OR REPLACE FUNCTION public.royal_wave82a_shield_selftest()
RETURNS TABLE(scenario text, result jsonb, shields_used_after int, boost_created boolean, boost_source text)
LANGUAGE plpgsql SECURITY DEFINER SET search_path TO 'public','auth'
AS $$
DECLARE
  uid uuid := gen_random_uuid();
  post_id uuid := gen_random_uuid();
  grant_id uuid;
  allow_id uuid;
  before_used int;
  after_used int;
  boost_row record;
  jwt_claims jsonb;
  res jsonb;
BEGIN
  -- 1. Seed synthetic auth user + profile + royal sub + post + crown + grant + allowance.
  INSERT INTO auth.users(id, aud, role, email, instance_id, created_at, updated_at, email_confirmed_at)
  VALUES (uid, 'authenticated', 'authenticated', 'w82a-'||uid||'@selftest.local',
          '00000000-0000-0000-0000-000000000000', now(), now(), now());

  INSERT INTO public.profiles(id, username) VALUES (uid, 'w82a_'||replace(uid::text,'-',''));

  INSERT INTO public.royal_pass_subscriptions(user_id, status, current_period_end)
  VALUES (uid, 'active', now() + interval '30 days');

  INSERT INTO public.posts(id, user_id, image_url, category)
  VALUES (post_id, uid, 'https://selftest.local/x.jpg', 'overall');

  INSERT INTO public.crowns(user_id, post_id, region_type, region_name, category, title, active)
  VALUES (uid, post_id, 'city', 'Selftest', 'overall', 'Selftest Crown', true);

  INSERT INTO public.royal_pass_grants(user_id, stripe_event_id, period_start, period_end,
    shields_granted, shekels_granted, boost_tokens_granted, status)
  VALUES (uid, 'evt_w82a_'||uid, now() - interval '1 day', now() + interval '30 days',
          5, 500, 3, 'granted')
  RETURNING id INTO grant_id;

  INSERT INTO public.royal_pass_shield_allowances(user_id, period_start, period_end,
    shields_granted, shields_used, royal_pass_grant_id)
  VALUES (uid, now() - interval '1 day', now() + interval '30 days', 5, 0, grant_id)
  RETURNING id INTO allow_id;

  -- Impersonate the test user for use_royal_shield (reads auth.uid()).
  jwt_claims := jsonb_build_object('sub', uid::text, 'role','authenticated');
  PERFORM set_config('request.jwt.claims', jwt_claims::text, true);
  PERFORM set_config('request.jwt.claim.sub', uid::text, true);
  PERFORM set_config('role', 'authenticated', true);

  -- === Scenario A: status = granted → success, +1 shield, 24h royal_pass boost. ===
  SELECT shields_used INTO before_used FROM public.royal_pass_shield_allowances WHERE id = allow_id;
  res := public.use_royal_shield(post_id);
  SELECT shields_used INTO after_used FROM public.royal_pass_shield_allowances WHERE id = allow_id;
  SELECT * INTO boost_row FROM public.boosts WHERE post_id = post_id AND boost_type='crown_shield'
    ORDER BY started_at DESC LIMIT 1;
  scenario := 'A_granted'; result := res; shields_used_after := after_used;
  boost_created := (boost_row.id IS NOT NULL AND boost_row.source = 'royal_pass'
                    AND boost_row.expires_at BETWEEN now()+interval '23 hours' AND now()+interval '25 hours');
  boost_source := boost_row.source;
  RETURN NEXT;

  -- === Scenario G: already-shielded → blocked, no credit consumed. ===
  before_used := after_used;
  res := public.use_royal_shield(post_id);
  SELECT shields_used INTO after_used FROM public.royal_pass_shield_allowances WHERE id = allow_id;
  scenario := 'G_already_shielded'; result := res; shields_used_after := after_used;
  boost_created := (after_used = before_used AND res->>'error' = 'already_shielded');
  boost_source := NULL;
  RETURN NEXT;

  -- Clear the shield so the remaining scenarios have a clean slate for the RPC to reach the grant-status gate.
  UPDATE public.boosts SET active=false, expires_at = now() - interval '1 second'
   WHERE post_id = post_id AND boost_type='crown_shield';

  -- === Scenarios B–E: non-granted lifecycle statuses all block, no credit consumed. ===
  FOR res IN SELECT to_jsonb(s.status) FROM (VALUES ('disputed'),('funds_withdrawn'),('reversed'),('refunded')) s(status)
  LOOP
    UPDATE public.royal_pass_grants SET status = res#>>'{}' WHERE id = grant_id;
    before_used := after_used;
    res := public.use_royal_shield(post_id);
    SELECT shields_used INTO after_used FROM public.royal_pass_shield_allowances WHERE id = allow_id;
    scenario := 'BCDE_'|| (SELECT status FROM public.royal_pass_grants WHERE id = grant_id);
    result := res;
    shields_used_after := after_used;
    boost_created := (after_used = before_used
                      AND res->>'error' = 'royal_benefits_temporarily_suspended');
    boost_source := NULL;
    RETURN NEXT;
  END LOOP;

  -- === Scenario F: orphan linkage (grant_id NULLed) → blocked. ===
  UPDATE public.royal_pass_grants SET status='granted' WHERE id = grant_id;
  -- NOT NULL constraint on royal_pass_grant_id — simulate orphan by deleting the linked grant row is blocked by FK.
  -- Instead: point allowance at a fresh grant whose row we then delete? FK is RESTRICT. So we relax constraint locally.
  ALTER TABLE public.royal_pass_shield_allowances ALTER COLUMN royal_pass_grant_id DROP NOT NULL;
  UPDATE public.royal_pass_shield_allowances SET royal_pass_grant_id = NULL WHERE id = allow_id;
  before_used := after_used;
  res := public.use_royal_shield(post_id);
  SELECT shields_used INTO after_used FROM public.royal_pass_shield_allowances WHERE id = allow_id;
  scenario := 'F_orphan_link'; result := res; shields_used_after := after_used;
  boost_created := (after_used = before_used AND res->>'error' = 'royal_allowance_not_linked');
  boost_source := NULL;
  RETURN NEXT;

  -- Restore link + NOT NULL for cleanup safety.
  UPDATE public.royal_pass_shield_allowances SET royal_pass_grant_id = grant_id WHERE id = allow_id;
  ALTER TABLE public.royal_pass_shield_allowances ALTER COLUMN royal_pass_grant_id SET NOT NULL;

  -- Cleanup: cascade from auth.users.
  PERFORM set_config('role','postgres', true);
  DELETE FROM public.boosts WHERE user_id = uid;
  DELETE FROM public.crowns WHERE user_id = uid;
  DELETE FROM public.royal_pass_shield_allowances WHERE user_id = uid;
  DELETE FROM public.royal_pass_grants WHERE user_id = uid;
  DELETE FROM public.royal_pass_subscriptions WHERE user_id = uid;
  DELETE FROM public.posts WHERE user_id = uid;
  DELETE FROM public.profiles WHERE id = uid;
  DELETE FROM auth.users WHERE id = uid;
END; $$;

REVOKE ALL ON FUNCTION public.royal_wave82a_shield_selftest() FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.royal_wave82a_shield_selftest() TO service_role;

-- Dispute strict-matching self-test.
CREATE OR REPLACE FUNCTION public.royal_wave82a_dispute_match_selftest()
RETURNS TABLE(scenario text, result jsonb, grant_status_after text, has_audit_restoration boolean)
LANGUAGE plpgsql SECURITY DEFINER SET search_path TO 'public','auth'
AS $$
DECLARE
  uid uuid := gen_random_uuid();
  grant_id uuid;
  res jsonb;
  audit_before int;
  audit_after int;
BEGIN
  INSERT INTO auth.users(id, aud, role, email, instance_id, created_at, updated_at, email_confirmed_at)
  VALUES (uid, 'authenticated','authenticated','w82a-dm-'||uid||'@selftest.local',
          '00000000-0000-0000-0000-000000000000', now(), now(), now());
  INSERT INTO public.profiles(id, username) VALUES (uid, 'w82adm_'||replace(uid::text,'-',''));

  -- Seed a disputed grant with a known dispute id + charge + PI + invoice.
  INSERT INTO public.royal_pass_grants(user_id, stripe_event_id, stripe_invoice_id,
    stripe_payment_intent_id, stripe_charge_id, stripe_dispute_id,
    period_start, period_end, shields_granted, shekels_granted, boost_tokens_granted,
    status, pre_dispute_status, disputed_at)
  VALUES (uid, 'evt_dm_'||uid, 'in_dm_'||uid, 'pi_dm_'||uid, 'ch_dm_'||uid, 'dp_correct_'||uid,
          now() - interval '1 day', now() + interval '30 days', 5, 500, 3,
          'disputed','granted', now())
  RETURNING id INTO grant_id;

  -- Scenario 1: correct charge, wrong dispute id → mismatch.
  res := public.handle_royal_dispute_reinstated(_stripe_charge_id := 'ch_dm_'||uid,
                                                _stripe_dispute_id := 'dp_wrong');
  scenario := 'wrong_dispute_by_charge'; result := res;
  SELECT status INTO grant_status_after FROM public.royal_pass_grants WHERE id = grant_id;
  has_audit_restoration := EXISTS(SELECT 1 FROM public.admin_audit_log
    WHERE (details->>'grant_id')::uuid = grant_id AND action = 'royal_dispute_reinstated');
  RETURN NEXT;

  -- Scenario 2: correct PI, missing dispute id → mismatch.
  res := public.handle_royal_dispute_reinstated(_stripe_payment_intent_id := 'pi_dm_'||uid,
                                                _stripe_dispute_id := NULL);
  scenario := 'missing_dispute_by_pi'; result := res;
  SELECT status INTO grant_status_after FROM public.royal_pass_grants WHERE id = grant_id;
  has_audit_restoration := EXISTS(SELECT 1 FROM public.admin_audit_log
    WHERE (details->>'grant_id')::uuid = grant_id AND action = 'royal_dispute_reinstated');
  RETURN NEXT;

  -- Scenario 3: correct invoice, wrong dispute id → mismatch.
  res := public.handle_royal_dispute_reinstated(_stripe_invoice_id := 'in_dm_'||uid,
                                                _stripe_dispute_id := 'dp_wrong2');
  scenario := 'wrong_dispute_by_invoice'; result := res;
  SELECT status INTO grant_status_after FROM public.royal_pass_grants WHERE id = grant_id;
  has_audit_restoration := EXISTS(SELECT 1 FROM public.admin_audit_log
    WHERE (details->>'grant_id')::uuid = grant_id AND action = 'royal_dispute_reinstated');
  RETURN NEXT;

  -- Scenario 4: correct dispute id → restores.
  res := public.handle_royal_dispute_reinstated(_stripe_charge_id := 'ch_dm_'||uid,
                                                _stripe_dispute_id := 'dp_correct_'||uid);
  scenario := 'correct_dispute'; result := res;
  SELECT status INTO grant_status_after FROM public.royal_pass_grants WHERE id = grant_id;
  has_audit_restoration := EXISTS(SELECT 1 FROM public.admin_audit_log
    WHERE (details->>'grant_id')::uuid = grant_id AND action = 'royal_dispute_reinstated');
  RETURN NEXT;

  -- Scenario 5: refunded grant cannot restore. Reset to refunded first.
  UPDATE public.royal_pass_grants SET status='refunded' WHERE id = grant_id;
  res := public.handle_royal_dispute_reinstated(_stripe_charge_id := 'ch_dm_'||uid,
                                                _stripe_dispute_id := 'dp_correct_'||uid);
  scenario := 'refunded_no_restore'; result := res;
  SELECT status INTO grant_status_after FROM public.royal_pass_grants WHERE id = grant_id;
  has_audit_restoration := false;
  RETURN NEXT;

  -- Cleanup
  DELETE FROM public.admin_audit_log WHERE (details->>'grant_id')::uuid = grant_id;
  DELETE FROM public.royal_pass_grants WHERE id = grant_id;
  DELETE FROM public.profiles WHERE id = uid;
  DELETE FROM auth.users WHERE id = uid;
END; $$;

REVOKE ALL ON FUNCTION public.royal_wave82a_dispute_match_selftest() FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.royal_wave82a_dispute_match_selftest() TO service_role;

-- Race harness: seeds a synthetic user and returns its id.
CREATE OR REPLACE FUNCTION public.royal_wave82a_race_seed(OUT uid uuid)
LANGUAGE plpgsql SECURITY DEFINER SET search_path TO 'public','auth'
AS $$
BEGIN
  uid := gen_random_uuid();
  INSERT INTO auth.users(id, aud, role, email, instance_id, created_at, updated_at, email_confirmed_at)
  VALUES (uid, 'authenticated','authenticated','w82a-race-'||uid||'@selftest.local',
          '00000000-0000-0000-0000-000000000000', now(), now(), now());
  INSERT INTO public.profiles(id, username) VALUES (uid, 'w82arace_'||replace(uid::text,'-',''));
END; $$;
REVOKE ALL ON FUNCTION public.royal_wave82a_race_seed() FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.royal_wave82a_race_seed() TO service_role;

-- Wrapper each racing session calls. Uses a shared advisory lock as a barrier
-- so both connections cross the start line together.
CREATE OR REPLACE FUNCTION public.royal_wave82a_race_call(
  _user_id uuid, _event_id text, _period_start timestamptz, _period_end timestamptz,
  _barrier_key bigint
) RETURNS jsonb
LANGUAGE plpgsql SECURITY DEFINER SET search_path TO 'public'
AS $$
DECLARE res jsonb;
BEGIN
  -- Both sessions take a shared advisory lock; neither proceeds until both hold it.
  PERFORM pg_advisory_lock_shared(_barrier_key);
  PERFORM pg_sleep(0.05);  -- ensure both have crossed
  res := public.grant_royal_monthly_benefits(
    _user_id, _event_id, 'in_race_'||_event_id,
    _period_start, _period_end, 999,
    'pi_race_'||_event_id, 'ch_race_'||_event_id, 'sub_race_'||_event_id);
  PERFORM pg_advisory_unlock_shared(_barrier_key);
  RETURN res;
END; $$;
REVOKE ALL ON FUNCTION public.royal_wave82a_race_call(uuid,text,timestamptz,timestamptz,bigint) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.royal_wave82a_race_call(uuid,text,timestamptz,timestamptz,bigint) TO service_role;

-- Cleanup helper.
CREATE OR REPLACE FUNCTION public.royal_wave82a_race_cleanup(_user_id uuid) RETURNS void
LANGUAGE plpgsql SECURITY DEFINER SET search_path TO 'public','auth'
AS $$
BEGIN
  DELETE FROM public.shekel_ledger WHERE user_id = _user_id;
  DELETE FROM public.boost_tokens_ledger WHERE user_id = _user_id;
  DELETE FROM public.wallets WHERE user_id = _user_id;
  DELETE FROM public.royal_pass_shield_allowances WHERE user_id = _user_id;
  DELETE FROM public.royal_pass_grants WHERE user_id = _user_id;
  DELETE FROM public.founder_grants WHERE user_id = _user_id;
  DELETE FROM public.profiles WHERE id = _user_id;
  DELETE FROM auth.users WHERE id = _user_id;
END; $$;
REVOKE ALL ON FUNCTION public.royal_wave82a_race_cleanup(uuid) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.royal_wave82a_race_cleanup(uuid) TO service_role;

-- Also grant EXECUTE to the sandbox_exec role so the harness can invoke via the exec-tool psql.
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname='sandbox_exec') THEN
    EXECUTE 'GRANT EXECUTE ON FUNCTION public.royal_wave82a_shield_selftest() TO sandbox_exec';
    EXECUTE 'GRANT EXECUTE ON FUNCTION public.royal_wave82a_dispute_match_selftest() TO sandbox_exec';
    EXECUTE 'GRANT EXECUTE ON FUNCTION public.royal_wave82a_race_seed() TO sandbox_exec';
    EXECUTE 'GRANT EXECUTE ON FUNCTION public.royal_wave82a_race_call(uuid,text,timestamptz,timestamptz,bigint) TO sandbox_exec';
    EXECUTE 'GRANT EXECUTE ON FUNCTION public.royal_wave82a_race_cleanup(uuid) TO sandbox_exec';
  END IF;
END; $$;
