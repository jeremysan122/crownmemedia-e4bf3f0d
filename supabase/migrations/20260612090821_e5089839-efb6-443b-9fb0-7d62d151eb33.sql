...
CREATE OR REPLACE FUNCTION public.send_dm_share(
  p_recipient_id uuid,
  p_kind text,                -- 'post_share' | 'profile_share'
...
BEGIN
  ...
  -- Recipient is reachable (not banned/suspended, exists)
  ...
  IF p_kind = 'post_share' THEN
    SELECT NOT COALESCE(is_removed, false)
       ...
       AND COALESCE(moderation_status, 'approved') NOT IN ('removed','rejected','quarantined')
      INTO v_post_ok
      FROM public.posts
      WHERE id = p_post_id;
  ...
  -- Dedupe: same sender → recipient → same target within 30s
  SELECT id INTO v_existing_id
    FROM public.messages
   WHERE sender_id = v_sender
     AND receiver_id = p_recipient_id
     AND kind = p_kind
  ...
  INSERT INTO public.messages (
    sender_id, receiver_id, body, kind, shared_post_id, shared_profile_id, delivered_at
  ) VALUES (
    v_sender, p_recipient_id, v_body, p_kind,
