...
BEGIN
  ...
  FOR status_val IN SELECT unnest(ARRAY['disputed','funds_withdrawn','reversed','refunded']) LOOP
    UPDATE public.royal_pass_grants SET status = status_val WHERE id = grant_id;
    before_used := after_used;
    res := public.use_royal_shield(_pid);
    SELECT shields_used INTO after_used FROM public.royal_pass_shield_allowances WHERE id = allow_id;
    scenario := 'BCDE_'|| status_val; result := res; shields_used_after := after_used;
    boost_created := (after_used = before_used AND res->>'error' = 'royal_benefits_temporarily_suspended');
    boost_source := NULL;
    RETURN NEXT;
  END LOOP;
  ...
  BEGIN DELETE FROM public.crowns WHERE user_id = uid; EXCEPTION WHEN others THEN NULL; END;
  BEGIN DELETE FROM public.royal_pass_shield_allowances WHERE user_id = uid; EXCEPTION WHEN others THEN NULL; END;
  BEGIN DELETE FROM public.royal_pass_grants WHERE user_id = uid; EXCEPTION WHEN others THEN NULL; END;
  BEGIN DELETE FROM public.royal_pass_subscriptions WHERE user_id = uid; EXCEPTION WHEN others THEN NULL; END;
  BEGIN DELETE FROM public.posts WHERE user_id = uid; EXCEPTION WHEN others THEN NULL; END;
