-- Re-grant anon's column-level SELECT allowlist that was collaterally
-- dropped by REVOKE SELECT ON public.posts FROM anon in the previous
-- migration. Column-level grants live under the same SELECT privilege as
-- table-level ones and are revoked together.
