...
-- 1) Peak viewers: server-computed, cannot be inflated by client
DROP FUNCTION IF EXISTS public.bump_live_battle_peak_viewers(uuid, integer);

CREATE OR REPLACE FUNCTION public.bump_live_battle_peak_viewers(_battle_id uuid)
RETURNS integer
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  SELECT status, peak_viewers INTO _status, _current
    FROM public.live_battles WHERE id = _battle_id;
  IF NOT FOUND THEN RAISE EXCEPTION 'battle_not_found'; END IF;
  IF _status = 'ended' THEN RETURN COALESCE(_current, 0); END IF;

  SELECT COUNT(*)::int INTO _computed
    FROM public.live_battle_viewers
   WHERE battle_id = _battle_id
     AND last_seen_at > now() - interval '60 seconds';

  IF _computed > COALESCE(_current, 0) THEN
    UPDATE public.live_battles
       SET peak_viewers = _computed
     WHERE id = _battle_id;
    RETURN _computed;
  END IF;
...
REVOKE ALL ON FUNCTION public.bump_live_battle_peak_viewers(uuid) FROM public;
GRANT EXECUTE ON FUNCTION public.bump_live_battle_peak_viewers(uuid) TO authenticated;

-- 2) Highlight RPC — real profile columns, computed display_name
...
-- 3) Analytics RPC — real profile columns
...
-- 4) Vote timeline RPC — 1-minute buckets with cumulative totals
    ...
    FROM (
      ...
      FROM (
        SELECT
          ...
          COUNT(*) FILTER (WHERE voted_for_id = _b.host_id)::int AS host_votes,
          COUNT(*) FILTER (WHERE voted_for_id = _b.opponent_id)::int AS opponent_votes
...
REVOKE ALL ON FUNCTION public.get_live_battle_vote_timeline(uuid) FROM public;
GRANT EXECUTE ON FUNCTION public.get_live_battle_vote_timeline(uuid) TO authenticated;
