...
BEGIN
  ...
  SELECT count(*) INTO audit_before_ok FROM public.admin_audit_log
   WHERE target_id = gid::text AND action ILIKE '%reinstat%';
  ...
  BEGIN DELETE FROM public.admin_audit_log WHERE target_id = gid::text; EXCEPTION WHEN others THEN NULL; END;
