...
import { safeBoolean, safeOptionalString, safeOptionalUrl, safeParse, z } from './_validate.ts'

const schema = z.object({
  won: safeBoolean(false),
  battle_url: safeOptionalUrl(),
  opponent_username: safeOptionalString(),
})

const Email = (raw: unknown) => {
  const { won, battle_url, opponent_username } = safeParse(schema, raw)
  return (
    <CrownMeEmail
