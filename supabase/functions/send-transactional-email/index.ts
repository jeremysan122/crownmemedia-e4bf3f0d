...
// Auth note: verify_jwt = true gates the Supabase gateway, but the anon key is
// itself a valid JWT — so we MUST additionally require an authenticated user
// (or service_role) here to prevent unauthenticated email-sending abuse.
...
Deno.serve(async (req) => {
  ...
  // Require an authenticated user (or service_role). Reject anon-key callers.
  ...
  const isAuthedUser = !!sub && role === 'authenticated'
  if (claimsErr || (!isServiceRole && !isAuthedUser)) {
    return new Response(JSON.stringify({ error: 'Unauthorized' }), {
      status: 401, headers: { ...corsHeaders, 'Content-Type': 'application/json' },
    })
  }
  ...
  // Anti-spam: non-service-role callers may only send to their own verified email
  // (unless the template defines a fixed `to` address). Prevents authenticated
  // users from sending arbitrary emails to third parties.
  if (!isServiceRole && !template.to) {
    const callerEmail = ((claimsData as any)?.claims?.email as string | undefined)?.toLowerCase()
    if (!callerEmail || callerEmail !== effectiveRecipient.toLowerCase()) {
      return new Response(
        JSON.stringify({ error: 'Recipient must match the authenticated user\'s email address' }),
